...
// ---------------------------------------------------------------------------
// Versión de Java — coincide con tus servicios backend (Java 17)
// ---------------------------------------------------------------------------
java {
...
dependencies {
    ...
    // ── Lombok (opcional, para clases de datos más limpias) ────────────────
...
serenity {
    // El título del reporte mostrado en la parte superior del reporte HTML
    projectName = "Sistema de Pedidos de Restaurante — Pruebas de API"
}
...
├── build.gradle                          # ← Configuración de build (Sección 2)
├── settings.gradle                       # ← Nombre del proyecto
├── gradle.properties                     # ← Propiedades compartidas de Gradle
│
├── src/
│   ├── main/java/com/restaurant/qa/
│   │   ├── actors/                       # ← Configuraciones de actores
│   │   │   └── ApiUser.java              #     Actores: Chef, Cliente, Admin
│   │   ├── abilities/                    # ← Habilidades personalizadas
│   │   │   └── AuthenticateAsKitchen.java
│   │   ├── tasks/                        # ← Tareas del Screenplay
│   │   │   ├── CreateOrder.java
│   │   │   ├── UpdateOrderStatus.java
│   │   │   ├── DeleteOrder.java
│   │   │   └── GetReport.java
│   │   ├── questions/                    # ← Preguntas del Screenplay
│   │   │   ├── TheOrder.java
│   │   │   ├── TheResponse.java
│   │   │   └── TheErrorResponse.java
│   │   ├── models/                       # ← POJOs de Request/Response
│   │   │   ├── OrderRequest.java
│   │   │   ├── OrderResponse.java
│   │   │   └── ErrorResponse.java
│   │   └── utils/                        # ← Utilidades compartidas
│   │       ├── Endpoints.java            #     Constantes de rutas
│   │       └── TestDataBuilder.java      #     Fábrica de datos de prueba
│   │
│   └── test/
│       ├── java/com/restaurant/qa/
│       │   ├── runners/                  # ← Runners de suite de pruebas
│       │   │   └── TestRunner.java
│       │   └── stepdefinitions/          # ← Definiciones de pasos de Cucumber
│       │       ├── OrderStepDefinitions.java
│       │       ├── ReportStepDefinitions.java
│       │       ├── SecurityStepDefinitions.java
│       │       └── CommonStepDefinitions.java
│       │
│       └── resources/
│           ├── serenity.conf             # ← Configuración de Serenity (Sección 3)
│           ├── logback-test.xml          # ← Configuración de logging
│           ├── cucumber.properties       # ← Propiedades de integración Cucumber
│           └── features/                 # ← Archivos Gherkin .feature
│               ├── order_service/
│               │   ├── HDU01_create_order.feature
│               │   ├── HDU02_list_orders.feature
│               │   ├── HDU03_type_errors.feature
│               │   ├── HDU04_soft_delete.feature
│               │   └── HDU07_state_conflicts.feature
│               ├── report_service/
│               │   └── HDU08_business_rules.feature
│               └── cross_cutting/
│                   ├── HDU05_error_structure.feature
│                   └── HDU06_security.feature
```
...
public class OrderStepDefinitions {
    ...
    @Given("que el catálogo de productos contiene al menos un producto activo con id {int}")
    public void productoExiste(int productId) {
        // Verificar que el producto existe vía el endpoint de menú
        SerenityRest.given()
            .baseUri(baseUrl)
            .when()
            .get("/menu")
    ...
    @When("se envía una solicitud POST a {string}")
    public void enviarSolicitudPost(String path) {
        SerenityRest.given()
            .baseUri(baseUrl)
    ...
    public void verificarCodigoDeEstado(int codigoEsperado) {
        assertThat(lastResponse().statusCode())
            .as("Código de estado HTTP")
            .isEqualTo(codigoEsperado);
    }

    @Then("debe incluir el header {string} con el patrón {string}")
    public void verificarPatronDeHeader(String header, String patron) {
        String valorHeader = lastResponse().header(header);
        assertThat(valorHeader)
            .as("Valor del header '%s'", header)
            .isNotNull()
            .matches(patron.replace("{uuid}", "[0-9a-f\\-]{36}"));
    }

    @Then("el cuerpo debe seguir la estructura ErrorResponse")
    public void verificarEstructuraErrorResponse() {
        var body = lastResponse().body();
        assertThat(body.jsonPath().getString("timestamp")).isNotNull();
        assertThat(body.jsonPath().getInt("status")).isPositive();
        assertThat(body.jsonPath().getString("error")).isNotBlank();
        assertThat(body.jsonPath().getString("message")).isNotBlank();
    }
}
```
